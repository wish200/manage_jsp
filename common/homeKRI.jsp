...
<script type="text/javascript" src="${ctx}/newrisk/js/common/verificationFlash.js"></script>
...
$(function(){
	    ...
	    paper.text(150,20,"${searchDto.comname}关键风险预警指标热力图").attr({"font-weight":"bold","font-size":"18px","fill":"#000000"});
...
function searchKRI(){
    ...
    asy_Ajax("${ctx}/jsp/common/windKRIsearch.jsp","","html",function(data){
...
function alertIndexAtt(data,indexName){
     closePopWind();
    asy_Ajax("${ctx}/jsp/monitorwarn/windIndexArrt.jsp",'',"html",function(ajaxData){
        $("body").append("<div id='popWindow'><div>");
...
function sendEmail(){
    if(confirm("确认发送给${session.userLogin.email}?")){
	   ...
	   var comcode='${searchDto.comCode}';
	    asy_Ajax('${ctx}/monitorwarn/index_sendEmailKPI.do',{'searchDto.statDate':statDate,'searchDto.comCode':comcode},'html',function(){
	       alert("邮件发送成功！");
	    });
   }
}
$(function(){
//表头固定
    var width=$("#KKRI>table").width();
    $(window).unbind("scroll");
    $(window).scroll(function(){
         var scroH = $(window).scrollTop();
         var navH = $(".report_form").offset().top; 
         if(scroH>0){
            $("#KKRI #form_tit").css({"position":"fixed","top":navH-scroH+"px","width":width}).show();
        	$("#KKRI #form_thead").css({"position":"fixed","top":navH-scroH+49+"px","width":width}).show();
         }else{
            if($("#KKRI").scrollTop()<=0){
            $("#KKRI #form_tit").css({"position":"static"});
        	$("#KKRI #form_thead").css({"position":"fixed"}).hide();
            }else{
            $("#KKRI #form_tit").css({"position":"fixed","top":navH+"px","width":width}).show();
        	$("#KKRI #form_thead").css({"position":"fixed","top":navH+49+"px","width":width}).show();
        	}
         }
    });
    $("#KKRI").unbind("scroll");
    $("#KKRI").scroll(function(){
        var scroH = $(this).scrollTop();
        var navH = $(".report_form").offset().top; 
        var widthArray = new Array();
        if (scroH>=5){
        	$("#KKRI>table thead tr td").each(function(){
        		 widthArray.push($(this).width());
        	});
        	for(var i=0;i<widthArray.length;i++){
        		$("#KKRI #form_thead").find("td").eq(i).width(widthArray[i]);
        	}
        	if($(window).scrollTop()>0){
        	$("#KKRI #form_tit").css({"position":"fixed","top":navH-$(window).scrollTop()+"px","width":width}).show();
        	$("#KKRI #form_thead").css({"position":"fixed","top":navH-$(window).scrollTop()+49+"px","width":width}).show();
        	}else{
        	$("#KKRI #form_tit").css({"position":"fixed","top":navH+"px","width":width}).show();
        	$("#KKRI #form_thead").css({"position":"fixed","top":navH+49+"px","width":width}).show();
        	}
...
//--打印
function wordpressOWK(){
   var printHtml=$('#KKRI');
   var printHtmlCopy=printHtml.clone();
   printHtmlCopy.find("#form_tit").css({"position":"static","width":"100%"});
   printHtmlCopy.find('#form_thead').hide();
   wordpress(printHtmlCopy.html());
}
//--查看全部
function checkKRIALL(){
     window.open("${ctx}/common/login_getHomeKRI.do?searchDto.doing=more");
}
</script >
</div>
  <div class="report_form" style="margin-top:-40px;position:relative">
       <div class="kri" style="float:left;width:26%;height:293px;">
	       	<!-- <h2 style="margin-left:27px;margin-top:10px">关键风险预警指标热力图</h2> -->
	           <div id="container" style="position:relative;float:left;text-align: center;height:293px;padding-top: 10px">
	           </div>
       </div>
       <div id="KKRI" style="position:relative;float:left;width:72.5%;height:293px;overflow-y:scroll;overflow-x:hidden;margin:0;padding:0;">
                    <div id="form_tit" class="form_tit"  >
                        <table>
                          <tr>
                            <th id="downTitle">&nbsp;&nbsp;${searchDto.comname}关键风险预警指标(${searchDto.statDate}) </th>
                            <th width="6.3%"><div><a href="javascript:checkKRIALL();" title="查看全部" class="open"></a></div></th>
                            <th width="6.3%"><div><a href="javascript:searchKRI();" title="搜索" class="search"></a></div></th>
                            <th width="6.3%"><div><a href="javascript:downloadOWKRI();" title="下载" class="download"></a></div></th>
                            <th width="6.3%"><div><a href="javascript:wordpressOWK();" title="打印" class="wordpress"></a></div></th>
   ...
   <s:if test="%{session.userLogin.ccDepPositionBaseDto.deppositioncode=='DP0000001'}">
       <!--右侧浮动-->
