	 ...
	 var setting = {
       ...
       for(var i=0;i<department.length;i++){
          if(temp.indexOf(department[i].value)!=-1){
	        ...
	        var dataChild={id:userCode[i].value,pId:department[i].value,name:userName[i].value,email:email[i].value};
...
function addRight(){
		  ...
		  for (var i=0, l=nodes.length;i<l; i++) {
		      ...
		      if(nodes[i].pId!=null){//不是部门的时候加入
			      ...
			      if(flag){
			           tempOption+="<li  dep='"+nodes[i].pId+"' remail='"+nodes[i].email+"' rname='"+nodes[i].name+"'   rid='"+nodes[i].id+"' '><span>"+nodes[i].name+"</span><a onclick='deleteUser(this)'>关闭</a></li>";
			      };
		      }
		  }
	      if(tempOption!=""){
...
function queDing(){
    ...
    var selecRid=new Array();
    $('#cTreeDiv ul li').each(function(){
        selecvalue.push($(this).attr('rname'));
        selecRid.push($(this).attr('remail')+'-'+$(this).attr('rname')+'-'+$(this).attr('rid'));
    });
    if('bccs'==userType){
         $(".bccsShow").eq(iiii).val(selecvalue);
         $(".bccs").eq(iiii).val(selecRid);
    }
	if('stcs'==userType){
	        $(".stcsShow").eq(iiii).val(selecvalue);
	        $(".stcs").eq(iiii).val(selecRid);
	 }
    closeDiv();
}
//
...
function innertRight(){
    var  _Caddressee='';
   if('bccs'==userType){
        _Caddressee=$.trim($(".bccs").eq(iiii).val());
  }
   if('stcs'==userType){
        _Caddressee=$.trim($(".stcs").eq(iiii).val());
   }
  if(_Caddressee!=''){
    var _CaddresseeArray=_Caddressee.split(',');
    var tempHtml="";
    for(var i=0;i<_CaddresseeArray.length;i++){
        if(_CaddresseeArray[i]!=''){
         tempHtml+="<li dep='" +_CaddresseeArray[i].split('-')[0]+"' rid='"+_CaddresseeArray[i].split('-')[2]+"'  remail='"+_CaddresseeArray[i].split('-')[0]+"'     rname='"+_CaddresseeArray[i].split('-')[1]+"'><span>"+_CaddresseeArray[i].split('-')[1]+"</span><a onclick='deleteUser(this)'>关闭</a></li>";
         }
    }
