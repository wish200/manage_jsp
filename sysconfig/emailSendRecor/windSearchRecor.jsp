...
function QueryT(){
	...
	$("#queryForm").ajaxSubmit({
        type : 'post',  
        url : '${ctx }/sysconfig/emailSend_list.do',
...
function Reset(){
    ...
    <div class="right">
        ...
        <ul>
            ...
            <li>
                <label>发送时间：</label>
                <input style="width:90px;border:1px solid #cccdce;line-height:20px;" name ="searchDto.startDate"  class="Wdate" onclick="WdatePicker({startDate:'<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"/>'})" class="text" type="text" />&nbsp;至
                <input style="width:90px;border:1px solid #cccdce;line-height:20px;" name ="searchDto.endDate" class="Wdate" onclick="WdatePicker({startDate:'<fmt:formatDate value="${now}" pattern="yyyy-MM-dd"/>'})"  class="text" type="text" />
            </li>
